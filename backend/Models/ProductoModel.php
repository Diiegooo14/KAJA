...
class ProductoModel
{
    public static function buscarPorId(int $id): ?array
    {
        ...
        $consulta = $pdo->prepare(
            'SELECT p.id, p.nombre, p.precioCoste, p.precioVenta, p.stock,
    ...
    public static function listarTodos(string $busqueda = '', ?int $idCategoria = null): array
    {
        $pdo = Database::connect();
        $sql = 'SELECT p.id, p.nombre, p.precioCoste, p.precioVenta, p.stock,
                        c.id AS idCategoria, c.nombre AS categoria
                        FROM PRODUCTO p
                        JOIN CATEGORIA c ON p.idCategoria = c.id
                        WHERE 1=1';
    ...
    public static function crear(array $datos): int
    {
        $pdo = Database::connect();
        $consulta = $pdo->prepare(
            'INSERT INTO PRODUCTO (idCategoria, nombre, precioCoste, precioVenta, stock)
            VALUES (:categoria, :nombre, :coste, :venta, :stock)'
        );
        $consulta->execute([
    ...
    {
        $pdo = Database::connect();
        $pdo->prepare('UPDATE PRODUCTO SET activo = 0 WHERE id = :id')
            ->execute([':id' => $id]);
    }
